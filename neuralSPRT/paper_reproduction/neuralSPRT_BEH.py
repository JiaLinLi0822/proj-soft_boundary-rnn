...
def fillTrace(x_range, y_center, y_se, face_color=(0.8, 0.8, 0.8), ax=None, alpha=0.2):
    """MATLAB-like fillTrace(X,Y,err,color,alpha): filled band + center line."""
    if ax is None:
        ax = plt.gca()
    ...
    err = err[pick]
    err[np.isnan(err)] = 0
    ax.fill_between(X, Y - err, Y + err, color=face_color, alpha=alpha, linewidth=0)
    ax.plot(X, Y, color=face_color, linewidth=1)
...
def linearFitErr(beta: np.ndarray, x: np.ndarray, y: np.ndarray, y_se: np.ndarray) -> float:
    ...
    pick = np.isfinite(y) & np.isfinite(y_se) & (y_se > 0)
    x = x[pick]
    y = y[pick]
    y_se = y_se[pick]
...
def neuralSPRT_BEH(info: Dict[str, Any], monkey_id: int, fig_switch: Union[int, List[int]]) -> Dict[str, Any]:
    ...
    if fig_flags[2]:
        ...
        if np.any(fig_flags[[3, 4, 5, 6]]):
            warnings.warn(
                "fig_switch includes 2 together with later panels; as in MATLAB, "
                "this sets lastAccumShape2Sac=0 globally and changes N*/psychometric/cumLLR results.",
                RuntimeWarning,
            )
    ...
    if fig_flags[4]:
        ...
        p_choice2 = LLR_choice2_dist / np.maximum(LLR_all_dist, 1)
    ...
    if fig_flags[5]:
        ...
        for j in [1, 2]:
            ...
            for ni in unique_N:
                ...
                std_n = np.std(tempM[pick_n, 1], ddof=1)
                bigS[pick_n] = std_n if std_n > 0 else np.nan
            ...
            if np.any(np.isinf(w)):
                ...
                w[np.isinf(w)] = (np.max(finite_w) * 1e6) if finite_w.size else 1e12
...
if __name__ == "__main__":
    ...
    for i in range(1, info["fig"]):
        plt.figure(i)
        plt.savefig(os.path.join(save_dir, f"neuralSPRT_BEH_MonkeyJ_fig{i}.png"), dpi=300, bbox_inches="tight")
        plt.savefig(os.path.join(save_dir, f"neuralSPRT_BEH_MonkeyJ_fig{i}.svg"), bbox_inches="tight")
